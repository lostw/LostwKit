...
public enum SwiftyRSA {
    ...
    static func generateRSAKeyPair(sizeInBits size: Int, applyUnitTestWorkaround: Bool = false) throws -> (privateKey: RSA.PrivateKey, publicKey: RSA.PublicKey) {
        ...
        let privateKey = try RSA.PrivateKey(reference: privKey)
        let publicKey = try RSA.PublicKey(reference: pubKey)
