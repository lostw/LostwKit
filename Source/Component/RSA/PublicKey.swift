...
extension RSA.PublicKey {
    ...
    public func encrypt(string: String) throws -> String {
        return try encrypt(data: string.data(using: .utf8)!, padding: .PKCS1).base64EncodedString()
    }
    ...
    /// - Throws: SwiftyRSAError
    public func encrypt(data: Data, padding: Padding = .PKCS1) throws -> Data {
        ...
        while idx < decryptedDataAsArray.count {
            ...
            let status = SecKeyEncrypt(reference, padding, chunkData, chunkData.count, &encryptedDataBuffer, &encryptedDataLength)
    ...
    public func verify(message: String, signature: String) throws -> Bool {
        let data = message.data(using: .utf8)!
        guard let signatureData = Data(base64Encoded: signature) else {
            throw SwiftyRSAError.dataToStringConversionFailed
        }

        return try verify(data: data, signatureData: signatureData)
    }
    ...
    public func verify(data: Data, signatureData: Data, digestType: RSA.DigestType = .sha1) throws -> Bool {
        ...
        let status = SecKeyRawVerify(reference, digestType.padding, digestBytes, digestBytes.count, signatureBytes, signatureBytes.count)
